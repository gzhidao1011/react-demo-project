...
#### 2.2.1 前端接口是否需要修改——结论与对照表

**结论**：
- **已有接口无需修改**：`/api/auth/**`（登录、注册、刷新、me）、`/api/chat/**`、`/api/user/locale` 的 URL 与用法保持不变；登录/注册仍走网关，仅后端由 auth-service 处理，前端无感。现有 [UserInfo](packages/services/src/auth.service.ts) 已含 `roles?: string[]`，无需改。
- **需要新增**：在 `packages/services` 中**新增**管理 API 模块（如 `user-management.service.ts` 或按资源拆文件），以及对应的类型、分页请求/响应，与后端 user-service 管理 API 一一对应。

**后端 API 与前端类型/接口对照**（与 [后端.md](后端.md) 2.4、后端 DTO 一致）：

| 后端方法 | 路径 | 前端 API（建议） | 请求体/查询 | 响应 data 类型 |
|----------|------|------------------|-------------|----------------|
| GET | `/users?page&size&sort&email&name&role&deleted` | getUserPage(params) | PageRequest + 筛选 | PagedResult\<UserDetailDto\>（items, total, page, size） |
| GET | `/users/:id` | getUserById(id) | — | UserDetailDto |
| POST | `/users` | createUser(body) | CreateUserRequest | UserDetailDto（201 + Location） |
| PUT | `/users/:id` | updateUser(id, body) | UpdateUserRequest | —（200） |
| DELETE | `/users/:id` | deleteUser(id) | — | —（204） |
| PATCH | `/users/:id/restore` | restoreUser(id) | — | —（200） |
| GET | `/roles?page&size&sort&deleted` | getRolePage(params) | PageRequest + deleted | PagedResult\<RoleDetailDto\> |
| GET | `/roles/:id` | getRoleById(id) | — | RoleDetailDto |
| POST | `/roles` | createRole(body) | CreateRoleRequest | RoleDetailDto（201 + Location） |
| PUT | `/roles/:id` | updateRole(id, body) | UpdateRoleRequest | —（200） |
| DELETE | `/roles/:id` | deleteRole(id) | — | —（204） |
| PATCH | `/roles/:id/restore` | restoreRole(id) | — | —（200） |
| PUT | `/roles/:id/permissions` | setRolePermissions(id, body) | UpdateRolePermissionsRequest | —（200） |
| GET | `/permissions` | getPermissions() | — | PermissionDto[] |

**请求/响应类型与后端 DTO 对齐**（JSON 字段名与后端 camelCase 一致）：
- **UserDetailDto**：id, name, email, phone, emailVerified, createdAt, updatedAt, deletedAt, roles（RoleSummaryDto[]）
- **RoleSummaryDto**：id, code, name
- **RoleDetailDto**：id, name, code, description, createdAt, updatedAt, deletedAt, permissions（PermissionDto[]）
- **PermissionDto**：id, resource, action, description
- **CreateUserRequest**：name, email, phone?, password, roleIds?
- **UpdateUserRequest**：name, email, phone?, roleIds?
- **CreateRoleRequest**：name, code, description?, permissionIds?
- **UpdateRoleRequest**：name, code, description?
- **UpdateRolePermissionsRequest**：permissionIds（Long[]）
- **PagedResult\<T\>**：items: T[], total: number, page: number, size: number

前端 baseURL 已为 `/api`，故上述接口调用路径为 `get("/users")`、`get("/users/" + id)` 等，无需再加 `/api` 前缀。
