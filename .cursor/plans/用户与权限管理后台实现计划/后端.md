...
- **角色管理**：
  ...
  - `POST /api/roles`：创建角色，成功返回 201 + `Location: /api/roles/{id}`；`PUT`/`DELETE`：更新/软删除角色；删除为软删除，可选 restore。
  - `PUT /api/roles/{id}/permissions`：为角色绑定权限（幂等替换）。
- **权限管理**：
...
- **user-service**：  
  ...
  - 返回 403 时与现有 [ResultCode.FORBIDDEN](services/api-common/src/main/java/com/example/api/common/ResultCode.java) 统一。  
