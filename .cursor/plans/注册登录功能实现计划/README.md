...
7.1 **[注册邮箱验证（Resend 实现）](./11-注册邮箱验证.md)** - ✅ 已实现
   - Resend 集成、硬验证模式、验证链接含 email 参数、登录页 EMAIL_NOT_VERIFIED 处理、verify-email 页完整流程

7.2 **[密码重置](./12-密码重置.md)** - 待实现
   - 忘记密码、重置密码流程，复用 Resend 邮件服务，与邮箱验证保持一致的架构模式
   - [12a-密码重置-后端](./12a-密码重置-后端.md)：后端 API、PasswordResetService、限流、Resend 邮件
   - [12b-密码重置-前端](./12b-密码重置-前端.md)：忘记密码页、重置密码页、auth.service
